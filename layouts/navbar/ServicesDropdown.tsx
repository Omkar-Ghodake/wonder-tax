...
const ServicesDropdown = ({ closeDropdown }: { closeDropdown: () => void }) => {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className='fixed left-1/2 -translate-x-1/2 bg-white shadow-2xl border border-gray-500/30 rounded-lg top-[10vh] hidden md:flex w-[85vw] xl:w-[70vw] h-[70vh]'
    ...
    >
      <div className='side-bar w-[30%] bg-gray-500/10'>
        {SERVICES_DROPDOWN_LINKS.map((item, idx) => (
          <div
            ...
            className={`cursor-pointer px-4 py-3 flex items-center justify-between hover:bg-primary/10 border-l-4 text-sm xl:text-base ${
      ...
      <div className='content p-5 w-[70%] space-y-5'>
        <div>
          <h2 className='title text-lg xl:text-xl text-primary-darken'>
            {selectedService.service.title}
          </h2>

          <p className='sub-text text-xs xl:text-sm'>
        ...
        <div className='sub-services grid grid-cols-3 gap-5'>
          {selectedService.service.subServices.map((subItem) => (
            ...
            >
              <IoMdCheckmarkCircleOutline className='text-xl xl:text-2xl w-fit text-primary-darken' />
              <span className='w-full font-semibold text-sm xl:text-base hover:text-primary-darken'>
                {subItem.title}
              </span>
            </Link>
          ))}
